...
"""
To be used in conjunction with run_preprocessing_test.py, but can also be used independently.
Code to extract a specific cohort of the .xml data as training data. This is specified by the TEST_DATA list in the config file.
This parameter specifies from folders .xml files should NOT be extracted.
...
config_train = Config(
    ...
    ALL_ATLASES = True,
    TRAIN_DATA = True,  # This is the critical difference to the run_preprocessing_test.py script.
    TEST_DATA = ["whitecatnii", "whiteCAT_updt","nss_updt", "NSSnii", "NUdatanii"]  # To avoid overlap, enusure this list is the same for run_preprocessing_test.py
)
